...
- Automated agent-skill security scanners on skill installers (e.g. Snyk, Socket — see Snyk's "ToxicSkills" research) run categories like Prompt Injection, Malicious Code, and — at High severity — Credential Handling and Secret Detection. Earlier revisions of this repository also hosted an unrelated personal skill collection (`a third-party skill collection`, under `.agents/skills/` and `.claude/skills/`, installed separately via its own `npx skills@latest add`) whose `wizard` skill legitimately generates setup scripts that capture API keys and write `.env`/GitHub Actions secrets — exactly the language a Credential Handling detector is built to catch. If a scanner evaluates the whole repository rather than the specific skill path being installed, that unrelated skill's legitimate behavior gets misattributed to `loop-runtime`. That unrelated collection has since been removed from this repository entirely (verified: `skills/engineering/loop-runtime/` and `.loop/` contain no credential-, secret-, or token-related content), specifically to close off this class of misattribution. If you fork or repackage this skill, keep it in a repository dedicated to nothing else — the fix is structural, not a one-time cleanup.
